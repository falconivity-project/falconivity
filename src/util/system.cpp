...
const char * const BITCOIN_CONF_FILENAME = "falconivity.conf";
...
#else
    const char* pszModule = "falconivity";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Falconivity";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Falconivity";
#else
    // Unix-like
    return pathRet / ".falconivity";
#endif
#endif
}
