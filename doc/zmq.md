...
The Falconivity Core daemon can be configured to act as a trusted "border
router", implementing the falconivity wire protocol and relay, making
...
The ZeroMQ feature in Falconivity Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building falconivityd:
...
For instance:
    ...
    $ falconivityd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/falconivityd.tx.raw \
...
These options can also be provided in falconivity.conf.
...
From the perspective of falconivityd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into falconivityd directly. Furthermore, no information is
...
using. Falconivityd appends an up-counting sequence number to each
